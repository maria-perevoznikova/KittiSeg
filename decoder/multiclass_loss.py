...
def loss(hypes, decoded_logits, labels):
    ...
    with tf.name_scope('loss'):
        ...
        if hypes['loss'] == 'xentropy':
            ...
            cross_entropy_mean = _compute_soft_iou(hypes, labels, softmax, epsilon)
        ...
        # only for logging
        soft_ui = _compute_soft_iou(hypes, labels, softmax, epsilon)

        losses = {}
        losses['total_loss'] = total_loss
        losses['xentropy'] = cross_entropy_mean
        losses['weight_loss'] = weight_loss
        losses['soft_iou'] = soft_ui
...
def _compute_soft_iou(hypes, labels, softmax, epsilon):
...
def evaluation(hyp, images, labels, decoded_logits, losses, global_step):
    ...
    eval_list = []
    eval_list.append(('total_loss', losses['total_loss']))
    eval_list.append(('cross_entropy', losses['xentropy']))
    eval_list.append(('weight_loss', losses['weight_loss']))
    eval_list.append(('soft_IoU', 1 - losses['soft_iou']))
