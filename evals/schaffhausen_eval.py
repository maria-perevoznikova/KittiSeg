...
def eval_image(hypes, gt_image, cnn_image):
    ...
    valid_gt = np.zeros(gt_classes[0].shape, dtype=bool)
    ...
    total_negnum = 0
    for gt in gt_classes:
        FN, FP, posNum, negNum = seg.evalExp(gt, cnn_image, thresh, validMap=None, validArea=valid_gt)
        total_fn += FN
        total_fp += FP
        total_posnum += posNum
        total_negnum += negNum

    return total_fn, total_fp, total_posnum, total_negnum
